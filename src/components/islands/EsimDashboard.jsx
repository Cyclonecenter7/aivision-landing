...
function Spark({ data, color, h = 22 }) {
  ...
  const id = 'sp' + color.replace('#', '').replace(/[(),.\s]/g, '') + Math.floor(Math.random() * 1e6);
  return (
...
function KpiCard({ label, value, sub, sparkData, sparkColor, accent, warn }) {
  const bg = accent ? C.brand : (warn ? ha(C.sun, .07) : T.card);
  const fg = accent ? '#fff' : T.t1;
  const fgm = accent ? 'rgba(255,255,255,.5)' : T.t2;
  return (
    <div style={{ background: bg, padding: '10px 12px 0', display: 'flex', flexDirection: 'column', flex: 1, minWidth: 0, ...ch(7) }}>
      <div style={{ fontSize: 8, fontWeight: 600, letterSpacing: '.13em', textTransform: 'uppercase', color: fgm, marginBottom: 2 }}>{label}</div>
      <div style={{ fontSize: 18, fontWeight: 800, color: fg, lineHeight: 1, fontVariantNumeric: 'tabular-nums', whiteSpace: 'nowrap' }}>{value}</div>
      {sub && <div style={{ fontSize: 10, color: fgm, marginTop: 1 }}>{sub}</div>}
      <div style={{ marginLeft: -12, marginRight: -12, marginTop: 'auto' }}>
        <Spark data={sparkData} color={accent ? 'rgba(255,255,255,.7)' : (sparkColor || C.brand)} h={20} />
      </div>
    </div>
  );
}
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
      <div style={{ display: 'flex', gap: 5 }}>
        <KpiCard accent label="Ветка 1 · Выручка" value="31 240 ₽" sub="За мес. 412 800 ₽" sparkData={s1} />
        <KpiCard label="Маржа В1" value="8 680 ₽" sub="~27% от выручки" sparkData={s1.map((v) => v * .28)} sparkColor={C.emerald} />
        <KpiCard label="Конверсия В1" value="40%" sub="Создан → Закрыт" sparkData={[36, 38, 37, 40, 39, 41, 40, 42, 40, 40]} sparkColor={C.emerald} />
        <KpiCard warn label="Конверсия В2" value="8%" sub="Создан → Закрыт" sparkData={[6, 7, 6, 8, 7, 9, 8, 8, 7, 8]} sparkColor={C.sun} />
      </div>
      ...
      <div style={{ background: T.card, padding: '10px 12px', ...ch(8) }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 8 }}>
          ...
          <span style={{ fontSize: 8, fontWeight: 600, letterSpacing: '.13em', textTransform: 'uppercase', color: T.t3 }}>Сравнение веток · 30 дней</span>
        </div>
        <div style={{ display: 'grid', gridTemplateColumns: '1fr auto 1fr', gap: 8, alignItems: 'center' }}>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
            <div style={{ fontSize: 8, fontWeight: 600, color: T.t3, letterSpacing: '.08em', textTransform: 'uppercase' }}>Ветка 1 · ₽</div>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
            <div style={{ fontSize: 8, fontWeight: 600, color: T.t3, letterSpacing: '.08em', textTransform: 'uppercase' }}>Ветка 2 · Крипта</div>
        ...
        <div style={{ marginTop: 8, padding: '6px 8px', background: ha(C.brand, .07), borderLeft: `2px solid ${C.brand}`, ...ch(5) }}>
          <span style={{ fontSize: 10, color: T.t2 }}>Ветка 1 доминирует по всем метрикам. Приоритет рекламного бюджета — </span>
          <span style={{ fontSize: 10, fontWeight: 700, color: C.brand }}>Ветка 1</span>
        </div>
      </div>

      <div style={{ background: T.card, padding: '10px 12px', ...ch(8) }}>
        <div style={{ fontSize: 8, fontWeight: 600, letterSpacing: '.12em', textTransform: 'uppercase', color: T.t3, marginBottom: 6 }}>Выручка по дням</div>
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8 }}>
          {[{ label: 'Ветка 1 (₽)', data: [22, 28, 18, 42, 55, 68, 60, 78, 65, 95], color: C.brand }, { label: 'Ветка 2 (Крипта)', data: [0, 0, 0, 0, 0, 0, 0, 100, 0, 0], color: C.indigo }].map((ch2) => (
            <div key={ch2.label}>
              <div style={{ fontSize: 7, color: T.t3, marginBottom: 3 }}>{ch2.label}</div>
              <div style={{ display: 'flex', alignItems: 'flex-end', gap: 2, height: 36 }}>
                {ch2.data.map((v, i) => <div key={i} style={{ flex: 1, height: v + '%', background: ch2.color, opacity: .4 + v / 200, borderRadius: 1 }} />)}
              </div>
            </div>
          ))}
        </div>
    ...
    <div style={{ background: T.card, padding: '10px 12px', ...ch(8) }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
        {[0, 1].map((vi) => {
          ...
          return (
            <div key={vi}>
              <div style={{ fontSize: 8, fontWeight: 600, color: T.t3, letterSpacing: '.08em', textTransform: 'uppercase', marginBottom: 6 }}>Ветка {vi + 1} · {vi === 0 ? '₽' : 'Крипта'}</div>
              {fn.map((s, i) => {
  ...
  return (
    <div style={{ background: T.card, ...ch(8), overflow: 'hidden' }}>
      <div style={{ padding: '8px 12px', borderBottom: `1px solid ${T.border}` }}>
        <span style={{ fontSize: 8, fontWeight: 600, letterSpacing: '.12em', textTransform: 'uppercase', color: T.t3 }}>Топ продуктов · Ветка 1 · по выручке</span>
      </div>
      ...
      <div style={{ padding: '8px 12px', borderTop: `1px solid ${T.border}`, background: T.card2 }}>
        <div style={{ fontSize: 8, fontWeight: 600, color: T.t3, letterSpacing: '.1em', textTransform: 'uppercase', marginBottom: 4 }}>Топ продуктов · Ветка 2</div>
